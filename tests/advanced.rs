...
fn texts(page: &Layout, expected: &str) -> usize {
    page.primitives
        .iter()
        .filter(|p| matches!(p, Primitive::Text { text, .. } if text == expected))
        .count()
}
fn glyphs(page: &Layout, expected: u32) -> usize {
    page.primitives
        .iter()
...
fn validates_explicit_connections() {
    ...
    assert!(layout(&t, Default::default())
        .unwrap_err()
        .message()
        .contains("endpoint"));
...
fn custom_meter_groups_and_explicit_beam_breaks_change_geometry() {
    ...
    let before = layout(&t, options).unwrap();
    t.measures[0].voices[0][1].annotations.beaming = Beaming::Break;
    let after = layout(&t, options).unwrap();
    let count = |p: &Layout| {
...
fn multi_measure_rests_condense_without_losing_addresses() {
    ...
    let page = layout(&t, options).unwrap();
    ...
    let split = layout(&t, options).unwrap();
...
fn score_tracks_support_independent_modes_meters_and_instrument_groups() {
    ...
    let standard = LayoutOptions {
        display: DisplayMode::Standard,
        ..Default::default()
    };
    let tabs = LayoutOptions {
...
fn staff_aware_document_draws_cross_staff_beams_and_validates_master_bars() {
    ...
    let page = layout_document(&document).unwrap();
    assert!(page
        .geometry
        .primitives
        .iter()
    ...
    assert!(layout_document(&document)
        .unwrap_err()
        .message()
...
fn render_style_and_revision_cache_are_shared_by_svg_output() {
    ...
    let first = cache
        .layout_or_try_build(7, || layout(&source, options))
        .unwrap()
        .to_svg();
    let second = cache
        .layout_or_try_build(7, || {
            Err(RenderError::new(
                ErrorKind::Internal,
    ...
    let mut individually_styled = layout(&source, options).unwrap();
...
fn background_layout_returns_the_requested_revision() {
    ...
    worker
        .request(REVISION, track(1), LayoutOptions::default())
        .unwrap();
    let result = worker
        .receive_timeout(std::time::Duration::from_secs(2))
        .unwrap();
    assert_eq!(result.revision, REVISION);
    assert_eq!(result.layout.unwrap().beats.len(), 4);
}
...
fn engraving_settings_apply_transposition_fingering_and_system_gap() {
    ...
    let mut invalid = LayoutOptions::default();
    invalid.engraving.slur_height = f32::NAN;
    assert!(layout(&source, invalid).is_err());
}
...
fn score_condenses_only_shared_rests_and_preserves_track_identity() {
    ...
    let page = layout_score(&[t.clone(), t.clone()], options).unwrap();
    ...
    let uncondensed = layout_score(&[t, played], options).unwrap();
...
fn metadata_is_measured_and_visibility_controls_apply() {
    ...
    let shown = layout(&t, Default::default()).unwrap();
    let hidden = layout(
        &t,
        LayoutOptions {
            show_metadata: false,
...
fn quarter_tone_accidentals_cancel_and_empty_diagrams_are_errors() {
    ...
    assert!(layout(&t, options).is_err());
}
